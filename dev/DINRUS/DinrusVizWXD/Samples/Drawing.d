...
// Define a new application тип, each program should derive a class from Приложение
class Drawing : public Приложение
{
public:
...
// `Main program' equivalent: the program execution "starts" here
    public override бул поИниц()
    {
        ...
        MyFrame рамка = new MyFrame("Пример с рисованием",
                                     Точка(50, 50), Размер(550, 340));

        // показ it and tell the application that it's our main окно
        рамка.показ(да);
        ТопОкно(рамка);
...
// Создай a new application object: this macro will разрешить wxWidgets to create
...
// Define a new рамка тип: this is going to be our main рамка
class MyFrame : public Рамка
{

    // рамка ructor
    this( ткст титул,  Точка поз,  Размер Размер)
    {
        super(null, wxID_ANY, титул, поз, Размер,
                     wxDEFAULT_FRAME_STYLE | ПОЛН_ПЕРЕРИС_ПРИ_ПЕРЕМЕРЕ);
        // set the рамка пиктограмма
...
//        menuFile.Append(File_ShowAlpha, "&Alpha screen\tF10");
        ...
        menuFile.добавьЧекЭлт(File_Clip, "&Clip\tCtrl-C", "Clip/unclip drawing");

        menuFile.добавьЧекЭлт(File_GraphicContext, "&Use GraphicContext\tCtrl-Y", "Use GraphicContext");

        menuFile.добавьРазделитель();
        menuFile.Append(File_About, "&About...\tCtrl-A", "показ about dialog");
        menuFile.добавьРазделитель();
        ...
        menuUserScale.добавьРазделитель();
        menuUserScale.Append( UserScale_Restore, "&Restore to normal\tCtrl-0" );

        Меню menuAxis = new Меню();
        menuAxis.добавьЧекЭлт( AxisMirror_Horiz, "Mirror горизонтально\tCtrl-M" );
        menuAxis.добавьЧекЭлт( AxisMirror_Vertic, "Mirror vertically\tCtrl-N" );
        ...
        menuColour.Append( Colour_TextForeground, "Text &переднийПлан..." );
        menuColour.Append( Colour_TextBackground, "Text &заднийПлан..." );
        menuColour.Append( Colour_Background, "Background &цвет..." );

        menuColour.добавьЧекЭлт( Colour_BackgroundMode, "&Opaque/transparent\tCtrl-B" );
        menuColour.добавьЧекЭлт( Colour_TextureBackgound, "Draw textured зад&ground\tCtrl-T" );
...
// handlers) which process them. It can be also done at run-time, кноп for the
// simple меню events like this the static метод is much simpler.

    собМеню      (File_Quit,     &OnQuit);
    собМеню      (File_About,    &OnAbout);
    собМеню      (File_Clip,     &OnClip);
    собМеню      (File_GraphicContext, &OnGraphicContext);



    собМенюДиапазон(MenuShow_First,   MenuShow_Last,   &OnShow);
    собМенюДиапазон(MenuOption_First, MenuOption_Last, &OnOption);
...
class MyCanvas: public ScrolledWindow
{
    ...
    super(родитель, wxID_ANY, ДЕФПОЗ, вхДефРазм,
                           wxHSCROLL | wxVSCROLL | ПОЛН_ПЕРЕРИС_ПРИ_ПЕРЕМЕРЕ);
...
//            if ( ! m_owner.m_backgroundBrush.Ок() ) 
            if ( m_owner.m_backgroundBrush is null ) 
            {
                Цвет цв = new Цвет(0,128,0);
                Кисть b = new Кисть(цв, FillStyle.wxSOLID);
                dc.Background(b);
            }
        }
...
*/
    ...
    проц OnMouseMove(Объект отправитель, Событие e)
    {
        ...
        КУКлиента dc = new КУКлиента(this);
        ПодготовьКР(dc);
        m_owner.ПодготовьКР(dc);
...
//        dc.DrawLine( x+20, y+50, 100, y+50 );
        ...
        dash1[2] = 5;  // Make секунда dash longer
        // ud.УстDashes( 6, dash1 );
        dc.pen( ud );
    ...
    проц DrawTestPoly(DC dc)
    {
        ...
        dc.DrawText(("The third star only есть a hatched outline"), 10, 50);
...
//        dc.DrawPolyPolygon( count.length, count, star2, 450, 150);
    ...
    проц DrawText(DC dc)
    {
        ...
        // use Швейцарский and not СемействоШрифтов.Нормальный as the latter can't be rotated
        // under Win9x (it is not TrueType)
        dc.шрифт( Шрифт.Швейцарский );
...
/+++++
    проц DrawAlpha(DC dc)
    {
        ...
        wx.common.Прям r = wx.common.Прям( cast(цел) маржин, cast(цел)(маржин+ширь*0.66), cast(цел) ширь, cast(цел)ширь) ;
        ...
        dc.DrawRoundedRectangle( 0 , cast(цел)(маржин + ширь / 2) , cast(цел) ширь * 3 , 100 , radius) ;
        
        dc.TextForeground( new Цвет(255,255,0 ) );
        dc.шрифт( new Шрифт( 40, СемействоШрифтов.wxSWISS, СтильШрифта.wxКурсивный, ВесШрифта.wxNORMAL ) );
...
+++++/
    ...
    проц DrawRegions(DC dc)
    {
        ...
        dc.DrawText(("The секунда copy should be identical кноп right part of it should be offset by 10 pixels."),
    ...
    */
        ...
        Цвет цв = new Цвет(255, 255, 0);
        Кисть yellowBrush = new Кисть(цв, FillStyle.wxSOLID);
        memdc2.Background( yellowBrush );
        memdc2.Clear();



        Pen yellowPen = new Pen(цв, 1, FillStyle.wxSOLID);

        // сейчас draw a white rectangle with red outline. It should
        // entirely eclipse the yellow заднийПлан.
        ...
        // Repeat, кноп draw directly on dc
    ...
    проц DrawGradients(DC dc)
    {
        ...
        Log tempLog = new Log();
        tempLog.LogMessage("Sorry! This function есть not been implemented by wxD.");
