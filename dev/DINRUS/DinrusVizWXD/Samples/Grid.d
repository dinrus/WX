...
private import stdrus;
	...
	public class GridFrame : Рамка
	{
		...
		public this()
		{
			super( null, -1, "wxWidgets grid class demo", ДЕФПОЗ, вхДефРазм );
			...
			файлМеню.Append( Cmd.ID_VTABLE, "&Virtual table test\tCtrl-V" );
			файлМеню.Append( Cmd.ID_BUGS_TABLE, "&Bugs table test\tCtrl-B" );
			файлМеню.Append( Cmd.ID_SMALL_GRID, "&Small Grid test\tCtrl-S" );
			файлМеню.добавьРазделитель();
			файлМеню.Append( Cmd.ID_EXIT, "E&xit\tAlt-X" );

			Меню viewMenu = new Меню();
			viewMenu.добавьЧекЭлт( Cmd.ID_TOGGLEROWLABELS,  "&Row labels", "" );
			viewMenu.добавьЧекЭлт( Cmd.ID_TOGGLECOLLABELS,  "&Col labels", "" );
			viewMenu.добавьЧекЭлт( Cmd.ID_TOGGLEEDIT,  "&Editable", "" );
			viewMenu.добавьЧекЭлт( Cmd.ID_TOGGLEROWSIZING, "Ro&w drag-resize", "" );
			viewMenu.добавьЧекЭлт( Cmd.ID_TOGGLECOLSIZING, "C&ol drag-resize", "" );
			viewMenu.добавьЧекЭлт( Cmd.ID_TOGGLEGRIDSIZING, "&Grid drag-resize", "" );
			viewMenu.добавьЧекЭлт( Cmd.ID_TOGGLEGRIDLINES, "&Grid Lines", "" );
			...
			viewMenu.добавьЧекЭлт( Cmd.ID_CELLOVERFLOW, "&Overflow cells", "" );
			viewMenu.добавьЧекЭлт( Cmd.ID_RESIZECELL, "&Resize ячейка (7,1)", "" );
			...
			colMenu.Append( Cmd.ID_GRIDLINECOLOUR, "&Grid строка цвет..." );
			colMenu.Append( Cmd.ID_SET_CELL_FG_COLOUR, "Уст ячейка &переднийПлан цвет..." );
			colMenu.Append( Cmd.ID_SET_CELL_BG_COLOUR, "Уст ячейка &заднийПлан цвет..." );

			Меню editMenu = new Меню();
			editMenu.Append( Cmd.ID_INSERTROW, "Insert &row" );
			editMenu.Append( Cmd.ID_INSERTCOL, "Insert &колонка" );
			editMenu.Append( Cmd.ID_DELETEROW, "Delete selected ro&ws" );
			editMenu.Append( Cmd.ID_DELETECOL, "Delete selected co&ls" );
			editMenu.Append( Cmd.ID_CLEARGRID, "Cl&ear grid ячейка contents" );

			Меню selectMenu = new Меню();
			selectMenu.добавьЧекЭлт( Cmd.ID_SELECT_UNSELECT, "Добавь new cells to the selection",
				"When off, old selection is deselected перед selecting the new cells" );
			...
			selectMenu.Append( Cmd.ID_SELECT_CELL, "Select ячейка (3, 1)" );
			...
			selectMenu.Append( Cmd.ID_DESELECT_CELL, "Deselect ячейка (3, 1)" );
			...
			менюСправки.Append( Cmd.ID_ABOUT, "&About wxGrid demo" );
			...
			logWin = new TextCtrl( this, -1, "", Точка( 0, gridH + 20 ), Размер( logW, logH ), TextCtrl.МНОГОСТРОК );
			...
			grid.УстCellValue( 0, 4, "Can вето edit this ячейка" );

			grid.УстCellValue( 0, 5, "Press\nCtrl+arrow\nto skip over\ncells" );

			grid.УстRowSize( 99, 60 );
			grid.УстCellValue( 99, 99, "Ctrl+End\nwill go to\nthis ячейка" );
			grid.УстCellValue( 1, 0, "This default ячейка will overflow into neighboring cells, кноп not if you turn overflow off." );
			...
			grid.УстCellValue( 4, 4, "a weird looking ячейка" );
			...
			grid.УстCellValue( 2, 4, "a wider колонка" );
			grid.УстColSize( 4, 120 );
			grid.УстColMinimalWidth( 4, 120 );

			grid.УстCellTextColour( 5, 8, Цвет.wxGREEN );
			grid.УстCellValue( 5, 8, "Bg from row attr\nText col from ячейка attr" );
			grid.УстCellValue( 5, 5, "Bg from row attr Text col from col attr and this текст is so long that it covers over many many empty cells кноп is broken by one that isn't" );
			...
			grid.УстCellValue( 7, 1, "Big бокс!" );

			ПеремерщикБокса topSizer = new ПеремерщикБокса( Ориентация.wxVERTICAL );
			...
			собМеню( Cmd.ID_TOGGLEROWLABELS,  & ToggleRowLabels ) ;
			собМеню( Cmd.ID_TOGGLECOLLABELS,  & ToggleColLabels ) ;
			собМеню( Cmd.ID_TOGGLEEDIT, & ToggleEditing ) ;
			собМеню( Cmd.ID_TOGGLEROWSIZING, & ToggleRowSizing ) ;
			собМеню( Cmd.ID_TOGGLECOLSIZING, & ToggleColSizing ) ;
			собМеню( Cmd.ID_TOGGLEGRIDSIZING, & ToggleGridSizing ) ;
			собМеню( Cmd.ID_TOGGLEGRIDLINES, & ToggleGridLines ) ;
			собМеню( Cmd.ID_AUTOSIZECOLS, & AutoSizeCols ) ;
			собМеню( Cmd.ID_CELLOVERFLOW, & CellOverflow ) ;
			собМеню( Cmd.ID_RESIZECELL, & ResizeCell ) ;
			собМеню( Cmd.ID_SETLABELCOLOUR, & УстLabelColour ) ;
			собМеню( Cmd.ID_SETLABELTEXTCOLOUR, & УстLabelTextColour ) ;
			собМеню( Cmd.ID_SETLABEL_FONT, & УстLabelFont ) ;
			собМеню( Cmd.ID_ROWLABELHORIZALIGN, & УстRowLabelHorizAlignment ) ;
			собМеню( Cmd.ID_ROWLABELVERTALIGN, & УстRowLabelVertAlignment ) ;
			собМеню( Cmd.ID_COLLABELHORIZALIGN, & УстColLabelHorizAlignment ) ;
			собМеню( Cmd.ID_COLLABELVERTALIGN, & УстColLabelVertAlignment ) ;
			собМеню( Cmd.ID_GRIDLINECOLOUR, & УстGridLineColour ) ;
			собМеню( Cmd.ID_INSERTROW, & InsertRow ) ;
			собМеню( Cmd.ID_INSERTCOL, & InsertCol ) ;
			собМеню( Cmd.ID_DELETEROW, & DeleteSelectedRows ) ;
			собМеню( Cmd.ID_DELETECOL, & DeleteSelectedCols ) ;
			собМеню( Cmd.ID_CLEARGRID, & ClearGrid ) ;
			собМеню( Cmd.ID_SELCELLS,  & SelectCells ) ;
			собМеню( Cmd.ID_SELROWS,  & SelectRows ) ;
			собМеню( Cmd.ID_SELCOLS,  & SelectCols ) ;
			
			собМеню( Cmd.ID_SET_CELL_FG_COLOUR, & УстCellFgColour ) ;
			собМеню( Cmd.ID_SET_CELL_BG_COLOUR, & УстCellBgColour ) ;
			
			собМеню( Cmd.ID_ABOUT, & About ) ;
			собМеню( Cmd.ID_EXIT, & OnQuit ) ;
			собМеню( Cmd.ID_VTABLE, & OnVTable) ;
			собМеню( Cmd.ID_BUGS_TABLE, & OnBugsTable) ;
			собМеню( Cmd.ID_SMALL_GRID, & OnSmallGrid) ;
			
			собМеню( Cmd.ID_DESELECT_CELL, & DeselectCell) ;
			собМеню( Cmd.ID_DESELECT_COL, & DeselectCol) ;
			собМеню( Cmd.ID_DESELECT_ROW, & DeselectRow) ;
			собМеню( Cmd.ID_DESELECT_ALL, & DeselectAll) ;
			собМеню( Cmd.ID_SELECT_CELL, & SelectCell) ;
			собМеню( Cmd.ID_SELECT_COL, & SelectCol) ;
			собМеню( Cmd.ID_SELECT_ROW, & SelectRow) ;
			собМеню( Cmd.ID_SELECT_ALL, & выделиВсе) ;
			собМеню( Cmd.ID_SELECT_UNSELECT, & OnAddToSelectToggle) ;
			
			собМеню( Cmd.ID_SET_HIGHLIGHT_WIDTH, & OnУстHighlightWidth) ;
			собМеню( Cmd.ID_SET_RO_HIGHLIGHT_WIDTH, & OnУстROHighlightWidth) ;
			
			собСеткаЯрлыкЛеваяНажата( & OnLabelLeftClick ) ;
			собСеткаЯчейкаЛеваяНажата( & OnCellLeftClick ) ;
			собСеткаРядРазмер( & OnRowSize ) ;
			собСеткаКолонкаРазмер( & OnColSize ) ;
			собСеткаВыделитьЯчейку( & OnSelectCell ) ;
			собСеткаДиапазонВыделение( & OnRangeSelected ) ;
			собСеткаЯчейкаИзменение( & OnCellValueChanged ) ;
			
			собСеткаРедакторПоказан( & OnEditorShown ) ;
			собСеткаРедакторСкрыт( & OnEditorHidden ) ;			
		...
		public проц УстLabelFont( Объект отправитель, Событие e )
		{
			...
			data.начальныйШрифт = grid.LabelFont;
			
			FontDialog fd = new FontDialog(this, data);
			
			if ( fd.ShowModal() == wxID_OK )
			{			
				Шрифт шрифт = fd.fontData.выбранныйШрифт;
				if ( шрифт.Ок )
				{
		...
		{
			BugsGridFrame рамка = new BugsGridFrame();
			рамка.показ( да );
		}		
		
		public проц OnSmallGrid( Объект отправитель, Событие e )
		{
			Рамка рамка = new Рамка( null, -1, "A Small Grid", ДЕФПОЗ, Размер( 640, 480 ) );
			Panel панель = new Panel( рамка, -1 );
			Grid agrid = new Grid( панель, -1, Точка( 10, 10 ), Размер( 400, 400 ), wxWANTS_CHARS | wxSIMPLE_BORDER );
		...
		public проц выделиВсе( Объект отправитель, Событие e )
		{
			grid.выделиВсе();
		}		
		...
		public проц OnSelectCell( Объект отправитель, Событие e )
		{
			...
			logBuf ~= "ячейка at row " ~ .вТкст(ge.Row) ~
				" col " ~ .вТкст(ge.Col) ~
		...
		public проц OnCellValueChanged( Объект отправитель, Событие e )
		{
			...
			logBuf = "Value changed for ячейка at" ~
				" row " ~ .вТкст(ge.Row) ~
				" col " ~ .вТкст(ge.Col);
		...
		public проц OnEditorShown( Объект отправитель, Событие e )
		{
			...
			if ( ( ge.Col == 4 ) &&
				( ge.Row == 0 ) &&
				( MessageBox( "Are you sure you wish to edit this ячейка",
		...
		public проц OnEditorHidden( Объект отправитель, Событие e )
		{
			...
			if ( ( ge.Col == 4 ) &&
				( ge.Row == 0 ) &&
				( MessageBox( "Are you sure you wish to finish editing this ячейка",
						"Checking", Dialog.wxYES_NO ) == Dialog.wxNO ) ) 
			{
	...
	public class BugsGridFrame : Рамка
	{
		public enum Columns
		{
			Col_Id,
		...
		public this()
		{
			super( null, -1, "Bugs table", ДЕФПОЗ, Размер( 500, 300 ) );
			Grid grid = new Grid( this, -1, ДЕФПОЗ );
		...
		public override ткст GetValue( цел row, цел col )
		{
			...
			switch( col )
			{
				...
				case Columns.Col_Opened:
					Log.LogError( "unexpected колонка" );
				break;
	...
	public class BigGridFrame : Рамка
	{
		private Grid m_grid;
		...
		{
			super(null, -1, "Plugin Virtual Table", ДЕФПОЗ, Размер( 500, 450 ) );
			m_grid = new Grid( this, -1, ДЕФПОЗ, вхДефРазм );
	...
	public class GridApp : Приложение
	{
		public override бул поИниц()
		{
			GridFrame рамка = new GridFrame();
			рамка.показ( да );
