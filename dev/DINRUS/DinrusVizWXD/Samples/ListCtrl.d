...
private import stdrus;
	...
	public class MyFrame : Рамка
	{
		...
		public this(ткст титул, Точка поз, Размер Размер)
		{
			...
			menuList.добавьРазделитель();
			menuList.Append(Cmd.LIST_SHOW_COL_INFO, _("показ &колонка info\tCtrl-C"));
			menuList.Append(Cmd.LIST_SHOW_SEL_INFO, _("показ &selected элтs\tCtrl-S"));
			menuList.добавьРазделитель();
			...
			menuList.добавьРазделитель();
			menuList.добавьЧекЭлт(Cmd.LIST_TOGGLE_LINES, _("Toggle &строки\tCtrl-I"));
			menuList.добавьЧекЭлт(Cmd.LIST_TOGGLE_MULTI_SEL, _("&Multiple selection\tCtrl-M"), _("Toggle multiple selection"));
			...
			m_logWindow = new TextCtrl(m_panel, wxID_ANY, "",
					ДЕФПОЗ, вхДефРазм,
					TextCtrl.МНОГОСТРОК | Окно.УТОПЛЕННЫЙ_БОРДЮР);
			...
			собМеню(Cmd.LIST_QUIT, &OnQuit);
			собМеню(Cmd.LIST_ABOUT, &OnAbout);
			собМеню(Cmd.LIST_LIST_VIEW, &OnListView);
			собМеню(Cmd.LIST_REPORT_VIEW, &OnReportView);
			собМеню(Cmd.LIST_ICON_VIEW, &OnIconView);
			собМеню(Cmd.LIST_ICON_TEXT_VIEW, &OnIconTextView);
			собМеню(Cmd.LIST_SMALL_ICON_VIEW, &OnSmallIconView);
			собМеню(Cmd.LIST_SMALL_ICON_TEXT_VIEW, &OnSmallIconTextView);
			собМеню(Cmd.LIST_VIRTUAL_VIEW, &OnVirtualView);
			
			собМеню(Cmd.LIST_FOCUS_LAST, &OnFocusLast);
			собМеню(Cmd.LIST_TOGGLE_FIRST, &OnToggleFirstSel);
			собМеню(Cmd.LIST_DESELECT_ALL, &OnDeselectAll);
			собМеню(Cmd.LIST_SELECT_ALL, &OnSelectAll);
			собМеню(Cmd.LIST_DELETE, &OnDelete);
			собМеню(Cmd.LIST_ADD, &OnAdd);
			собМеню(Cmd.LIST_EDIT, &OnEdit);
			собМеню(Cmd.LIST_DELETE_ALL, &OnDeleteAll);
			собМеню(Cmd.LIST_SORT, &OnSort);
			собМеню(Cmd.LIST_SET_FG_COL, &OnУстFgColour);
			собМеню(Cmd.LIST_SET_BG_COL, &OnУстBgColour);
			собМеню(Cmd.LIST_TOGGLE_MULTI_SEL, &OnToggleMultiSel);
			собМеню(Cmd.LIST_SHOW_COL_INFO, &OnShowColInfo);
			собМеню(Cmd.LIST_SHOW_SEL_INFO, &OnShowSelInfo);
			собМеню(Cmd.LIST_FREEZE, &OnFreeze);
			собМеню(Cmd.LIST_THAW, &OnThaw);
			собМеню(Cmd.LIST_TOGGLE_LINES, &OnToggleLines);
			
			собОбновиИП(Cmd.LIST_SHOW_COL_INFO, &OnUpdateShowColInfo);
			собОбновиИП(Cmd.LIST_TOGGLE_MULTI_SEL, &OnUpdateToggleMultiSel);
		...
		public проц RecreateList(long флаги, бул withText)
		{
			...
			m_listCtrl = new MyListCtrl(m_panel, Cmd.LIST_CTRL,
					ДЕФПОЗ, вхДефРазм,
					флаги |
					УТОПЛЕННЫЙ_БОРДЮР | ListCtrl.wxLC_EDIT_LABELS);
		...
		public проц InitWithVirtualItems()
		{
			...
			m_listCtrl.InsertColumn(1, _("секунда Column"));
		...
		public проц OnShowColInfo(Объект отправитель, Событие e)
		{
			...
			Log.LogMessage(.вТкст(count) ~ " колонки:");
			for ( цел c = 0; c < count; c++ )
			{
				Log.LogMessage("\tcolumn " ~ .вТкст(c) ~ " есть ширь " ~ .вТкст(m_listCtrl.GetColumnWidth(c)));
			}
		}
	...
	public class MyListCtrl : ListCtrl
	{
		...
		public this( Окно родитель, цел ид, Точка поз, Размер Размер, long стиль )
		{
			...
			собСписокНачалоТяга(Cmd.LIST_CTRL, &OnBeginDrag);
			собСписокНачалоПТяга(Cmd.LIST_CTRL, &OnBeginRDrag);
			собСписокНачниРедактированиеЯрлыка(Cmd.LIST_CTRL, &OnBeginLabelEdit);
			собСписокЗавершиРедактированиеЯрлыка(Cmd.LIST_CTRL, &OnEndLabelEdit);
			собСписокУдалиЭлт(Cmd.LIST_CTRL, &OnDeleteItem);
			собСписокУдалиВсеЭлты(Cmd.LIST_CTRL, &OnDeleteAllItems);
			собСписокЭлтВыделен(Cmd.LIST_CTRL, &OnSelected);
			собСписокЭлтДеселектирован(Cmd.LIST_CTRL, &OnDeselected);
			собСписокКлавишаВнизу(Cmd.LIST_CTRL, &OnListKeyDown);
			собСписокЭлтАктивирован(Cmd.LIST_CTRL, &OnActivated);
			собСписокЭлтФокусирован(Cmd.LIST_CTRL, &OnFocused);
			
			собСписокКолНажата(Cmd.LIST_CTRL, &OnColClick);
			собСписокКолПраваяНажата(Cmd.LIST_CTRL, &OnColRightClick);
			собСписокКолНачалоТяга(Cmd.LIST_CTRL, &OnColBeginDrag);
			собСписокКолТяг(Cmd.LIST_CTRL, &OnColDragging);
			собСписокКолКонецТяга(Cmd.LIST_CTRL, &OnColEndDrag);
			
			собСписокКэшХинт(Cmd.LIST_CTRL, &OnCacheHint);
		...
		public проц OnColBeginDrag(Объект отправитель, Событие e)
		{
			...
			if ( le.Column == 0 )
			{
				Log.LogMessage("Resizing this колонка shouldn't work.");
		...
		public проц OnSelected(Объект отправитель, Событие e)
		{
			...
			if ( (ФлагиСтиля & ListCtrl.wxLC_REPORT) > 0 )
			{
				...
				if ( GetItem(info) )
				{
					Log.LogMessage("Value of the 2nd поле of the selected элт: " ~
						info.Text);
				}
				else
				{
		...
		public ткст OnGetItemText(long элт, long колонка)
		{
			ткст s = "Column " ~ .вТкст(колонка) ~ " of элт " ~ .вТкст(элт);
			return s;
		}
		
		//---------------------------------------------------------------------	

		public цел приПолученииРисункаЭлта(long элт)
		{
			return 0;
		}
		...
		public ListItemAttr приПолученииАтраЭлта(long элт)
		{
		...
		public проц InsertItemInReportView(цел i)
		{
			...
			buf = "Элемент " ~ .вТкст(i) ~ " in колонка 2";
			УстItem(i, 2, buf);
		}
	}
	...
	public class ListCtrlApp : Приложение
	{
		public override бул поИниц()
		{
			MyFrame рамка = new MyFrame("ListCtrl Test", Точка(50, 50), Размер(450,340));
			рамка.показ(да);
