...
struct ArrayList /* for .NET compatibility */
{
	...
	public class MyFrame : MDIParentFrame
	{
		...
		public static MyFrame рамка = null;
		...
		public this( Окно родитель, ткст титул, Точка поз, Размер Размер )
		{
			...
			textWindow = new TextCtrl( this, -1, "A help окно", ДЕФПОЗ, вхДефРазм, TextCtrl.МНОГОСТРОК | TextCtrl.УТОПЛЕННЫЙ_БОРДЮР );
			...
			собМеню( Cmd.MDI_ABOUT, & OnAbout ) ;
			собМеню( Cmd.MDI_NEW_WINDOW, & OnNewWindow ) ;
			собМеню( Cmd.MDI_QUIT, & OnQuit ) ;
		...
		public проц OnNewWindow( Объект отправитель, Событие e )
		{
			...
			MyChild subframe = new MyChild( this, "Canvas Рамка", Точка( -1, -1 ),
	...
	public class MyCanvas : ScrolledWindow
	{
		...
		{
			super( родитель, -1, поз, Размер,  /*ПБордюр.*/УТОПЛЕННЫЙ_БОРДЮР | ПОЛН_ПЕРЕРИС_ПРИ_ПЕРЕМЕРЕ | wxVSCROLL | wxHSCROLL );
			ЦветФона = new Цвет( "WHITE" );
			
			//this.собМышь( & OnEvent ) ;
		}
	...
	public class MyChild : MDIChildFrame
	{
		...
		public static ткст s_title = "Canvas Рамка";
		
		//---------------------------------------------------------------------
		
		public this( MDIParentFrame родитель, ткст титул, Точка поз, Размер Размер, long стиль )
		{
			super( родитель, -1, титул, поз, Размер, стиль | ПОЛН_ПЕРЕРИС_ПРИ_ПЕРЕМЕРЕ );
			...
			ткст atitle = "Canvas Рамка " ~ .вТкст( ++MyFrame.gs_nFrames );
			...
			option_menu.Append( Cmd.MDI_CHANGE_TITLE, "Change &титул...\tCtrl-T" );
			option_menu.добавьРазделитель();
			option_menu.Append( Cmd.MDI_CHANGE_POSITION, "Сдвинь рамка\tCtrl-M" );
			option_menu.Append( Cmd.MDI_CHANGE_SIZE, "Resize рамка\tCtrl-S" );
			...
			//CreateStatusBar();
			MyFrame.рамка.StatusText = титул;
			...
			собМеню( Cmd.MDI_CHILD_QUIT, & OnQuit ) ;
			собМеню( Cmd.MDI_REFRESH,  & OnRefresh ) ;
			собМеню( Cmd.MDI_CHANGE_TITLE, & OnChangeTitle ) ;
			собМеню( Cmd.MDI_CHANGE_POSITION, & OnChangePosition ) ;
			собМеню( Cmd.MDI_CHANGE_SIZE, & OnChangeSize ) ;
			
			собМеню( Cmd.MDI_NEW_WINDOW, & MyFrame.рамка.OnNewWindow ) ;
			собМеню( Cmd.MDI_QUIT, & MyFrame.рамка.OnQuit ) ;
			собМеню( Cmd.MDI_ABOUT, & MyFrame.рамка.OnAbout ) ;
		...
		{
			ткст титул = GetTextFromUser( "Enter the new титул for MDI child", "MDI sample question", s_title, родитель.родитель );
		...
		public проц приПеремере( Объект отправитель, Событие e )
		{
			...
			Log.LogStatus( "Размер from event: %dx%d, from рамка %dx%d, клиент %dx%d",
		...
		public проц OnMove( Объект отправитель, Событие e )
		{
			...
			Log.LogStatus( "position from event: ({0}, {1}), from рамка ({2}, {3})",
	...
	public class MDI : Приложение
	{
		public override бул поИниц()
		{
			MyFrame.рамка = new MyFrame( null, "MDI Demo", Точка( -1, -1 ), Размер( 500, 400 ) );
			MyFrame.рамка.показ( да );
