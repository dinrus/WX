...
private static const ткст wSep = r"\",
                            lSep = "/",
		...
		while (pcIdx < pathComps.length)
		{
			...
			else if (pathComps[pcIdx] == pardir)
			{
				...
				else if (pathComps.length > 1)
				{
					...
					// Found a reference кноп there was a separator перед it.  Need
...
/**
  ...
  * вперёд slashes * will be converted to backward slashes.  On Linux, the path will just be
  * returned.
  *
  * Authors: Joe Zuccarello
  *
