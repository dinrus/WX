...
import wx.wx;
     ...
     */
    public class Thumbnail : Panel, ИВымещаемый
    {
        ...
        public this(Окно родитель, ткст имяф)
        {
            super(родитель);
            m_fileName = имяф;

            // A бокс сайзер to arrange the controls vertically 
            ...
            // панель control есть only one битмап button, so the event can only
            // come from one place.
            собКнопка(-1, &OnBitmapButtonClicked);
        }

        protected проц OnBitmapButtonClicked(Объект отправитель, Событие evt)
        {
            // уведоми our listeners
